...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_lscustomilegal:getVehiclesPricesImported', function(vehicles)
		ESX.TriggerServerCallback('esx_lscustomilegal:getVehiclesPrices', function(vehicles)
			Vehicles = vehicles
		end)
	end)
end)
...
RegisterNetEvent('esx_lscustomilegal:installMod')
AddEventHandler('esx_lscustomilegal:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustomilegal:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustomilegal:cancelInstallMod')
AddEventHandler('esx_lscustomilegal:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_lscustomilegal:installMod')
				else
					local vehiclePrice = 200
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_lscustomilegal:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_lscustomilegal:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == Config.JobName) or not Config.IstunnersJobOnly then
				for k,v in pairs(Config.Zones) do
